	...
	public void 장바구니담기(Item newItem, PrincipalDetail principal) {
		...
		Cart cart = cartRepository.findByUserId(user.getId()) // 사용자 userId로 사용자 장바구니 찾기
				.orElseGet(() -> { // 장바구니가 없다면 생성
					Cart newCart = Cart.builder().user(user).count(0).build();
		...
		cartItemRepository.findByCartIdAndItemId(cart.getId(), item.getId()) // cartId와 itemId로 cartItem을 불러온다.
				.map(existingCartItem -> { // CartItem이 존재할 경우 수량 추가
					if ((existingCartItem.getCount() + newItem.getCount()) > item.getStock()) { // 장바구니에 담긴 상품 개수가 재고보다
																								// 많으면 재고 최대량에 맞게
						cart.setCount(cart.getCount() + item.getStock() - existingCartItem.getCount()); // 기존 장바구니 총 개수
																										// + 해당 상품 재고 수
																										// + 기존 해당 상품을
																										// 담아놓은 개수
						...
						existingCartItem.setCount(existingCartItem.getCount() + newItem.getCount()); // 재고보다 적으면 선택한
																										// 개수만큼 장바구니에 추가
					...
					CartItem newCartItem = CartItem.builder().cart(cart).item(item).count(newItem.getCount()).build();
